...
import { AuthModel, UserModel, ApiResponse } from '@/types/auth'; // Ensure path
import * as authHelper from './auth-helpers';

export const login = async (email: string, password: string, identifier: string = 'device-uuid') => {
...
export const getUser = async () => {
  ...
  const response = await apiRequest<ApiResponse<UserModel>>('GET', API_ENDPOINTS.ADMIN_GET_USER);
